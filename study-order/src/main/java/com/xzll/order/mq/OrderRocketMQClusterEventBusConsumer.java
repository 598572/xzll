...
@Slf4j
@Component
public class OrderRocketMQClusterEventBusConsumer implements RocketMQClusterEventListener, InitializingBean {
	...
	public void handleEvent(String topicName, ClusterEvent clusterEvent) {
		...
		if (XZLL_TEST_TOPIC.equals(topicName)) {
			...
			log.info("订单服务接收到的data数据:{}",data);
			log.info("订单服务进行业务逻辑消费topicName:{}", topicName);
		}
	}
}
